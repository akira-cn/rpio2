...
Object.assign(Gpio.prototype, {
  ...
  read: function(buf, length){
    if(buf === undefined){
      ...
      if(this.activeLow){
        ...
        return rpio.readbuf(this.pin, newBuf, length);
      }else{
        return rpio.readbuf(this.pin, buf, length);
      }
    }
  },
  write: function(buf, length){
    if(length === undefined && buf === 0 || buf === 1){
      ...
      buf = new Buffer(buf.split('').map(function(i){return 0|i}));
      if(this.activeLow){
        var newBuf = new Buffer(length || buf.length);
        for(var i = 0; i < newBuf.length; i++){
          newBuf[i] = 1 ^ buf[i];
        }
        return rpio.writebuf(this.pin, newBuf, length);
      }else{      
        return rpio.writebuf(this.pin, buf, length);
      }
